...
it("initial dashboard is fail closed and exposes labeled search while loading", () => {
  vi.stubGlobal("sessionStorage", { getItem: () => null });
  const html = renderToStaticMarkup(createElement(ResultsDashboard));
  expect(html).toContain("조회 권한 확인 중");
  expect(html).toContain("결과를 불러오고 있습니다");
  expect(html).toContain('for="result-search"');
  expect(html).not.toContain("전화번호 전체 표시");
  expect(html).not.toContain("전체 조회 ·");
  expect(html).not.toContain("rd-record");
});
...
const sampleSubmission: ResultSubmission = {
  ...
  blobPathname: "sample-only",
  counselorName: "TEST",
  respondentPhone: "",
  participant: { name: "TEST", birthDate: "", age: null },
  answers: { 3: 5, 999: 2 },
  result: {
    primaryType: 1,
    primaryTypeName: "TEST",
    primaryTypeTitle: "TEST",
    wingType: 9,
    wingCode: "1w9",
    wingName: "TEST",
    wingTitle: "TEST",
...
describe("TXT button access boundary", () => {
  it("clearly labels TXT and disables it for restricted access", () => {
    for (const hasFullAccess of [false, true]) {
      const html = renderToStaticMarkup(
        createElement(TxtDownloadButton, {
  ...
  it("rejects even a manually invoked restricted button handler", () => {
    ...
    const button = TxtDownloadButton({
      submission: sampleSubmission,
      hasFullAccess: false,
    });
...
describe("JSON download boundary", () => {
  it("downloads the enriched JSON with raw answers and existing filename intact", async () => {
    ...
    expect(link.download).toBe(
      "mind-compass-result-2026-01-01-SYNTHETIC-ONLY.json"
    );
    ...
    expect(payload.answersByType[0].answers[0]).toMatchObject({
      questionId: 3,
      score: 5,
    });
...
describe("grouped answer viewer (synthetic only)", () => {
  it("renders canonical text, original id and score in nine groups", () => {
    const html = renderToStaticMarkup(
      createElement(GroupedAnswerDetails, {
        answers: { 3: 5, 999: 2 },
        hasFullAccess: true,
      })
    );
